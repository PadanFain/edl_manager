...
class EDLBaseHandler:
    ...
    def _kv_list(self, service, collection, query=None, limit=None,
        ...
        if skip:  params["offset"] = str(skip)
        if sort:  params["sort"]   = json.dumps(sort)
...
# =============================================================================
# Concrete base — combines EDLBaseHandler with PSCA
# =============================================================================
def make_handler_base():
    """
    Returns a base class combining EDLBaseHandler with
    PersistentServerConnectionApplication.

    Wrapped in a function so the PSCA import only happens at runtime inside
    Splunk — allowing the module to be imported in test environments without
    a full Splunk installation.
    """
    try:
        from splunk.persistconn.application import PersistentServerConnectionApplication

        class _Base(EDLBaseHandler, PersistentServerConnectionApplication):
            def __init__(self, command_line, command_arg):
                PersistentServerConnectionApplication.__init__(
                    self, command_line, command_arg)

        return _Base

    except ImportError:
        logger.warning(
            "PersistentServerConnectionApplication not available. "
            "Using EDLBaseHandler without PSCA. Expected in test environments."
        )
        return EDLBaseHandler


# Module-level base class used by all concrete handlers:
#   class IOCHandler(SplunkRestHandler): ...
SplunkRestHandler = make_handler_base()
