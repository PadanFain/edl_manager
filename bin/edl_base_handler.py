...
Key differences from BaseRestHandler:
  ...
  • handle(in_bytes) receives raw bytes from splunkd — JSON-decode to get request dict
  • query and headers arrive as [[key,val],...] lists — normalised to dicts in handle()
  • Return a dict {'payload': dict, 'status': int} — PSCA serialises payload itself
  • No self.response / self.request objects — request context comes from in_bytes
  • KV Store calls use splunklib.client.Service built from session.authtoken

Works on: Splunk Enterprise 9.0+, Splunk 10.x, Splunk Cloud (after AppInspect vetting)
Verified on: Splunk 10.4.0
"""
...
class EDLBaseHandler:
    ...
    def handle(self, in_bytes):
        ...
        in_bytes is raw bytes from splunkd. JSON-decode, normalise query/headers,
        then dispatch by HTTP method.
        """
        if isinstance(in_bytes, dict):
            request = in_bytes
        else:
            try:
                request = json.loads(in_bytes)
            except (json.JSONDecodeError, TypeError, ValueError) as exc:
                return {"payload": {"error": f"Could not parse request: {exc}"}, "status": 400}

        # Normalise query: PSCA sends [[key,val],...] — flatten to {key:val,...}
    ...
    def _ok(self, body):
        return {"payload": body, "status": 200,
                "headers": {"Content-Type": "application/json"}}

    def _created(self, body):
        return {"payload": body, "status": 201,
                "headers": {"Content-Type": "application/json"}}
    ...
    def _error(self, status, message, details=None):
        ...
        return {"payload": body, "status": status,
                "headers": {"Content-Type": "application/json"}}
